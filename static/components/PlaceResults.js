...
const PlaceResults = ({ placesData, selectedPlace, setSelectedPlace }) => {
  ...
  if (placesData && placesData.results) {
    ...
    placesData.results = placesData.results.sort(function (left, right) {
      const leftHasCMSMetric =
        left['Infection Rating'] ||
        left['Infection Rating'] === 0 ||
        left['Summary star rating'] ||
        left['Summary star rating'] === 0
      const rightHasCMSMetric =
        right['Infection Rating'] ||
        right['Infection Rating'] === 0 ||
        right['Summary star rating'] ||
        right['Summary star rating'] === 0
      return leftHasCMSMetric ? -1 : rightHasCMSMetric ? 1 : 0
    })
  }

  const placeTiles = placesData.results ? (
    placesData.results.map((place, i) => {
      ...
      return (
        <div style={selectedPlaceStyle} onClick={() => setSelectedPlace(place)}>
            ...
            <div>
              <b>
                ...
                <span style={{ color: '#fdcc0d' }}>
                  {getHaiStars(place['Infection Rating'])}
                </span>
              </b>
            </div>
          ) : (
          ...
          place['Summary star rating'] === 0 ? (
            <div>
              <b>
                Patient Rating:{' '}
                <span style={{ color: '#fdcc0d' }}>
                  {getHCAHPSStars(place['Summary star rating'])}
                </span>
              </b>
            </div>
          ) : (
  ...
  return <div style={{ width: '250px', marginRight: '15px' }}>{placeTiles}</div>
}
